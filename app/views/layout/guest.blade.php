  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav">
        <li {{ (Request::is('/') ? 'class="active"' : '') }} ><a href="{{ URL::route('home') }}">首页 </a></li>
        <li {{ (Request::is('dates') ? 'class="active"' : '') }}><a href="{{ URL::route('dates') }}">以往开彩成绩</a></li>
         <li class="dropdown">
          ...
          <ul class="dropdown-menu">
             ...
             <li {{ (Request::is('myNumber') ? 'class="active"' : '') }} ><a href="{{ URL::route('myNumber')}}">我的神字</a></li>
             <li role="separator" class="divider"></li>
             <li {{ (Request::is('sudoku') ? 'class="active"' : '') }} ><a href="{{ URL::route('sudoku')}}">万字四宫格</a></li>
             <li {{ (Request::is('analytics') ? 'class="active"' : '') }} ><a href="{{ URL::route('analytics')}}">频密出现万字分析</a></li>

          </ul>
        </li>
        <li {{ (Request::is('tbg') ? 'class="active"' : '') }} ><a href="{{ URL::route('tbg') }}">大伯公千字图</a></li>

      </ul>

        <form class="navbar-form navbar-left" action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
            <input type="hidden" name="cmd" value="_s-xclick">
            <input type="hidden" name="hosted_button_id" value="ZUY9SKQ2ARS2U">
            <input type="hidden" name="return" value="https://www.fourdhuat.com/tips/success">
            <input name="notify_url" value="https://www.fourdhuat.com/PayPal_IPN" type="hidden">
            <input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donate_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
            <img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1">
        </form>

        <form style="display:none" class="navbar-form navbar-left" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post" target="_top">
        <input type="hidden" name="cmd" value="_s-xclick">
      ...
      <ul class="nav navbar-nav navbar-right">
                <li ><a class="blink_text" href="{{ URL::route('tips') }}">打赏抽大奖 !!</a></li>

        @if (Auth::check())


                        <li><a href="{{ URL::route('logout') }}"><img style="width:25px;heigh:25px" alt="{{ Config::get('4dhuat.name') }}" src=" {{ Auth::user()->image }}"> 登出</a></li>

        @else
                       <li><a href="{{ URL::route('login') }}">脸书登入</a></li>
       @endIf
      </ul>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
